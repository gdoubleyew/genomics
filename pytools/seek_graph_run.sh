...
if true; then
  python create_queries.py -min 20 -max 40 -c 2,4,6,8,10 -o q20_40 -i human_go_slim.gmt
  python create_queries.py -min 40 -max 100 -c 2,4,6,8,10 -o q40_100 -i human_go_slim.gmt
  python create_queries.py -min 100 -max 300 -c 2,6,10,14,18 -o q100_300 -i human_go_slim.gmt
fi
...
if true; then
  ...
  time LD_LIBRARY_PATH=./lib ./bin/SeekMiner -x dataset.map -i gene_map.txt -d db.combined -p prep.combined -P platform.combined -Q quant2 -n 1000 -b 200 -u sinfo.combined -R dataset_size -V CV -I LOI -z z_score -m -M -O -q $query_file -o $tmp_dir
